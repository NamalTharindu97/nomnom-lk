...
func SetupRouter(cfg *config.Config, db *gorm.DB, rdb *redis.Client, log zerolog.Logger) (*gin.Engine, *services.CronService) {
	...
	notificationService := services.NewNotificationService(notificationRepo, deviceTokenRepo, offerRepo, &cfg.Firebase)
	...
	authHandler := handlers.NewAuthHandler(authService, firebaseService, auditService, &cfg.Browser, &cfg.JWT, &cfg.DemoViewer)
	...
	notificationHandler.SetOfferRepo(offerRepo)
	...
	v1 := r.Group("/api/v1")
	{
		...
		authGroup.Use(middleware.RateLimit(rdb, 20, 1*time.Minute, "rl:auth"))
		{
			...
			if cfg.DemoViewer.Enabled {
				authGroup.POST("/browser/demo", middleware.RateLimitStrict(rdb, 5, time.Minute, "rl:demo:ip"), authHandler.BrowserDemo)
			} else {
				authGroup.POST("/browser/demo", authHandler.BrowserDemo)
			}
		...
		adminGroup.Use(middleware.AuditTrail(auditService))
		{
			...
			adminGroup.GET("/analytics/device-stats", adminHandler.AnalyticsDeviceStats)
			adminGroup.GET("/analytics/recent-activity", adminHandler.AnalyticsRecentActivity)
		...
		portfolioReads := v1.Group("/admin")
		portfolioReads.Use(middleware.RateLimit(rdb, 60, time.Minute, "rl:admin-read"))
		portfolioReads.Use(middleware.Auth(cfg.JWT.Secret))
		portfolioReads.Use(middleware.RequireActive(userRepo))
		portfolioReads.Use(middleware.RequireRole("admin", "portfolio_viewer"))
		{
			portfolioReads.GET("/stats", adminHandler.Stats)
			portfolioReads.GET("/stats/timeline", adminHandler.StatsTimeline)
			portfolioReads.GET("/analytics/top-restaurants", adminHandler.AnalyticsTopRestaurants)
			portfolioReads.GET("/analytics/top-offers", adminHandler.AnalyticsTopOffers)
			portfolioReads.GET("/analytics/offer-stats", adminHandler.AnalyticsOfferStats)
			portfolioReads.GET("/analytics/expiring-offers", adminHandler.AnalyticsExpiringOffers)
			portfolioReads.GET("/categories", categoryHandler.List)
			portfolioReads.GET("/cuisine-tags", cuisineTagHandler.List)
			portfolioReads.GET("/order-platforms", orderPlatformHandler.List)
			portfolioReads.GET("/social-platforms", socialPlatformHandler.List)
			portfolioReads.GET("/banners", bannerHandler.List)
		}
