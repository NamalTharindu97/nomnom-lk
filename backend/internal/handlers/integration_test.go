...
func TestIntegration_LoginLockout_AfterTenFailures(t *testing.T) {
	...
	for i := 1; i <= 10; i++ {
		...
		w = testutil.PerformRequest(engine, http.MethodPost, "/api/v1/auth/login", bytes.NewBuffer(loginBody), "")
		assert.Equal(t, http.StatusUnauthorized, w.Code, "attempt %d should be 401", i)
	}
...
func TestIntegration_LoginLockout_BrowserLoginAlsoLocked(t *testing.T) {
	engine, _, err := testutil.Setup()
	require.NoError(t, err)

	email := fmt.Sprintf("lockout-browser-%d@test.com", time.Now().UnixNano())
	...
	for i := 1; i <= 10; i++ {
		...
		w := testutil.PerformRequest(engine, http.MethodPost, "/api/v1/auth/browser/login", bytes.NewBuffer(loginBody), "")
		assert.Equal(t, http.StatusUnauthorized, w.Code, "attempt %d should be 401", i)
	}
...
func TestIntegration_RateLimitByEmail_LoginThrottled(t *testing.T) {
	...
	for i := 1; i <= 5; i++ {
		w := testutil.PerformRequest(engine, http.MethodPost, "/api/v1/auth/login", bytes.NewBuffer(loginBody), "")
		assert.NotEqual(t, http.StatusTooManyRequests, w.Code, "attempt %d should not be rate limited yet", i)
	}
